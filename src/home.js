...
import menuPageLoading from "./menu";
import aboutPageLoading from "./about";
...
export default function homePageLoading() {
	...
	function handleOverlayButtonClick(event) {
		if (event.target.classList.contains("left-overlay-btn")) {
			menuPageLoading();
		} else if (event.target.classList.contains("homeBtn")) {
			homePageLoading();
		} else if (event.target.classList.contains("right-overlay-btn")) {
			aboutPageLoading();
		}
	}

	overLayButtons.forEach((button) => {
		button.addEventListener("click", handleOverlayButtonClick);
	});
}
